...
function initLoadingScreen() {
    ...
    // Hide loading screen after animation (reduced from 3000ms to 2000ms)
    setTimeout(() => {
...
function initScrollAnimations() {
    ...
    elementsToAnimate.forEach((el, index) => {
        ...
        // Reduced delay from 0.1s to 0.05s per element
        el.style.transitionDelay = `${index * 0.05}s`;
        observer.observe(el);
    });

    // Special faster animation for education cards
    educationCards.forEach((card, index) => {
        card.style.transition = 'all 0.3s ease'; // Even faster for education
        card.style.transitionDelay = `${index * 0.08}s`; // Minimal delay between cards
    });

    // Stagger animations for skill tags - much faster now
    const skillTags = document.querySelectorAll('.skill-tag');
    skillTags.forEach((tag, index) => {
        tag.style.opacity = '0';
        tag.style.transform = 'translateY(20px)';
        tag.style.transition = 'all 0.3s ease'; // Reduced from 0.5s
        
        setTimeout(() => {
            tag.style.opacity = '1';
            tag.style.transform = 'translateY(0)';
        }, 30 * index); // Reduced from 100ms to 30ms per tag
    });
}
...
function initContactCards() {
    ...
    // Faster animation for each card appearing
    contactCards.forEach((card, index) => {
        card.style.opacity = '0';
        card.style.transform = 'translateY(20px)';
        card.style.transition = 'all 0.3s ease';
        
        // Cards appear quickly one after another
        setTimeout(() => {
            card.style.opacity = '1';
            card.style.transform = 'translateY(0)';
        }, index * 50); // Reduced timing between cards
...
function initTimelineAnimation() {
    ...
    timelineItems.forEach(item => {
        ...
        item.style.transition = 'all 0.4s ease'; // Reduced from 0.6s
